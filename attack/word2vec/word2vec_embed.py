...
class Word2VecSubstitute():
    ...
    def __init__(self, tar_tokens=None, cosine=True, k = 30, threshold = 0.5, device = None):
        """
    ...
    def substitute(self, word, sent_token, word_pos, sub_word_pos):
        ...
        # Filter the meaning with nearby words by perplexity
        new_sents = []
        for _id in new_idx:
            if sub_word_pos is None:
                new_tokens = sent_token[:word_pos] + [self.id2word[_id]] + sent_token[word_pos + 1:]
            else:
                sub_tokens = sent_token[word_pos].split()
                sub_token = sub_tokens[:sub_word_pos] + [self.id2word[_id]] + sub_tokens[sub_word_pos + 1:]
                sub_token = "_".join(sub_token)
                new_tokens = sent_token[:word_pos] + [sub_token] + sent_token[word_pos + 1:]

            x_prime_sent = " ".join(new_tokens)
            x_prime_sent = x_prime_sent.replace("_", " ")
            new_sents.append(x_prime_sent)

        if len(new_sents) == 0:
            return []
        ppl = perplexity.compute(predictions=new_sents, model_id='gpt2')

        num_slt = min(len(new_sents), 7)
        new_idx = np.array(new_idx)[np.array(ppl['perplexities']).argsort()][:num_slt]
